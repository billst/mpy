...
namespace MiniPython1.walkers
{
    class AstFuncDef : AstSemantics2
    {
        ...
        public AstFuncDef(ContainersAndCurrentObjects containersArg, bool isCallFromFunctionCall = false)
            : base(containersArg)
        {
        ...
        public override void CaseAMinusExpression(AMinusExpression node)
        {
            ...
            if (node.GetLeft() != null)
            {
                ...
                leftType = containers.Current_TypeAttribute;
                if (leftType.Name != "number" || leftType.Name != "string" || leftType.Name !="general") throw new Exception("Minus operands must be numbers or strings at node: " + node.GetLeft().ToString());
            }
            if (node.GetRight() != null)
            {
                node.GetRight().Apply(this);
                rightType = containers.Current_TypeAttribute;
                if (rightType.Name != "number" || rightType.Name != "string" || rightType.Name != "general") throw new Exception("Minus operands must be numbers or strings");
            }
